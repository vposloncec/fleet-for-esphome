...
- **[WORKITEMS-1.7.3.md](WORKITEMS-1.7.3.md)** — **Current release (hotfix branch off `main`, parallel to `develop`'s 1.8 cycle).** Firmware-archive fix for ESPHome 2026.7's native ESP-IDF toolchain.
- **[WORKITEMS-1.8.md](WORKITEMS-1.8.md)** — LLM assistance (in progress on `develop`; not affected by this hotfix except that #243 needs merging back in — see 1.7.3's note).
